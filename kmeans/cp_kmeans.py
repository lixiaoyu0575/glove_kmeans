...
glove_dic = {}
with open("./../data/model_152m_50.vector") as f:
    for line in f:
        lis = line.split("\n")[0].split("\t")
        glove_dic[lis[0]] = [float(item) for item in lis[1:51]]
        # print lis
seq_vecs = []
with open("./../data/output2glove_152m.txt") as f:
    for line in f:
        seq = line.split("\n")[0].split(" ")
        seq_vec = np.zeros(50)
        for a in seq:
            if glove_dic.has_key(a):
                seq_vec += glove_dic[a]
        seq_vecs.append(seq_vec)
        # print seq_vec
X = np.array(seq_vecs)
print "got kmeans input"
...
file=open('./../result/kmeans_3_labels.txt','w')
